...
public class VertoonbaarApplicatie {
	...
	private int maakKeuzeUitMenu(int nrMenu) {
		...
		do {
			...
			try {
				...
				if (keuze < 1 || keuze > aantalKeuzes)
					throw new IllegalArgumentException();
			} catch (InputMismatchException | IllegalArgumentException e) {
				System.out.printf("Geef een geheel getal tussen 1 en %d in!%n", aantalKeuzes);
				invoer.nextLine();
			}
	...
	private void voegVoorstellingToe() {
		...
		try {
			...
			System.out.println("Aantal sterren en jaar moet een geheel getal zijn");
